function [wallet,sum,stats,result_detect]=coins_detect2(myImage,SF)
%% This function is for the provided dataset

%% 1 Detect shadows
I_hsv = rgb2hsv(myImage);
...
for i=1:size(stats,1)
    ...
    % valeur of green divsion is the key to distinguish gold and copper.the
    ...
    if (20.5538/SF)<diameter(i)&&diameter(i)<(22.5635/SF)
        cent=round(stats(i).Centroid);
        valPixelR=Ies(cent(1,2)-100:cent(1,2)+100,cent(1,1)-100:cent(1,1)+100,1);
        valPixelG=Ies(cent(1,2)-100:cent(1,2)+100,cent(1,1)-100:cent(1,1)+100,2);
        aveR=mean(valPixelR(:));
    ...
    if (22.5635/SF)<=diameter(i)&&diameter(i)<=(25.348/SF)
        cent=round(stats(i).Centroid);
        valPixelR=Ies(cent(1,2)-100:cent(1,2)+100,cent(1,1)-100:cent(1,1)+100,1);
        valPixelG=Ies(cent(1,2)-100:cent(1,2)+100,cent(1,1)-100:cent(1,1)+100,2);
        aveR=mean(valPixelR(:));
    ...
    % 2euro
    if (25.348/SF)<diameter(i)&&diameter(i)<=(28.35/SF)
